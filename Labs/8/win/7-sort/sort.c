...
DWORD WINAPI ThreadFunc(LPVOID lpParameter)
{
	...
	while ((td.threadId % (2 * pow2) == 0) && (td.threadId + pow2 < NO_THREADS))
	{
		/* TODO - merge chunks */
		dwRet = WaitForSingleObject(threads[td.threadId + pow2], INFINITE);
		DIE(dwRet == WAIT_FAILED, "WaitForSingleObject");
		thData[td.threadId].lpChunk = MergeChunks(thData[td.threadId].lpChunk, thData[td.threadId + pow2].lpChunk);
		pow2 = pow2 * 2;
	}
...
HANDLE init_setup(LPSTR filename)
{
	...
	for (i = 0; i < NO_THREADS; i++) {
		...
		threads[i] = CreateThread(
				...
				ThreadFunc,
				&thData[i],
				CREATE_SUSPENDED, /* wait before starting threads */
				NULL
				);
		DIE(threads[i] == NULL, "CreateThread");
	}

	/* start threads all at once */
	for (i = 0; i < NO_THREADS; i++) {
		dwRet = ResumeThread(threads[i]);
		DIE(dwRet == (DWORD)-1, "ResumeThread");
	}
