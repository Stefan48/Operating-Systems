...
void *thread_routine_y(void *arg)
{
	...
	for (i = data_y.start; i < data_y.stop && !finished; i++) {
		...
		if (local_found) {
			finished = 1;
			pthread_mutex_lock(&data_y.mutex); // lock here
			data_y.found = 1;
			data_y.val = i;
			pthread_mutex_unlock(&data_y.mutex); // unlock here
		} else {
...
void *thread_routine_x(void *arg)
{
	...
	for (i = data_x.start; i < data_x.stop && !finished; i++) {
		...
		if (local_found) {
			finished = 1;
			pthread_mutex_lock(&data_x.mutex); // lock here
			data_x.found = 1;
			data_x.val = i;
			pthread_mutex_unlock(&data_x.mutex); // unlock here
		} else {
			pthread_mutex_lock(&data_y.mutex);
			if (data_y.found)
				finished = 1;
